...
const statusConfig = {
  pending: { badge: <Badge variant="pending">CHỜ NỘP</Badge>, amountClass: 'text-amber-400' },
  waiting: { badge: <Badge variant="waiting">CHỜ KHAI</Badge>, amountClass: 'text-ink-3' },
  overdue: { badge: <Badge variant="overdue">QUÁ HẠN</Badge>, amountClass: 'text-red-400' },
};

export function TaxSummaryCard() {
  return (
    <SectionCard
      title={<><Receipt size={13} /> Nợ thuế nhà nước</>}
      action={
        <Link to="/thue">
          ...
          return (
            <div key={r.id} className="flex items-center justify-between py-2.5 border-b border-white/5 last:border-0">
              ...
              <div className="flex items-center gap-2">
                {r.remaining != null ? (
                  <span className={clsx('font-mono text-[11px] font-medium', s.amountClass)}>
